...
def growthfac(z, cosmo, power):
    ...
    if (
        np.abs(cosmo.omegamI + cosmo.omegavI + cosmo.omegakI + cosmo.omegaQI - 1.0)
        > 1.0e-10
    ):
    ...
    if cosmo.omegaQI == 0.0:
        ...
        omegamAA = cosmo.omegam / (
            AA + cosmo.omegam * (1.0 - AA) + cosmo.omegav * (AA3 - AA)
        )
        omegavAA = (
            AA3
            * cosmo.omegav
        ...
        growthfac = (
            ...
            * omegamAA
            / (
                omegamAA ** (4.0 / 7.0)
                - omegavAA
                + (1.0 + omegamAA / 2.0) * (1.0 + omegavAA / 70.0)
            )
        )
    else:
        growthfac = fgQQ(z, power) / (1.0 + z)
...
def Tgrowthfac(z, cosmo, power):
    ...
    Tgrowthfac = growthfac(z, cosmo, power) / growthfac(0.0, cosmo, power)
