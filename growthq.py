...
def growthfacQ(ln_a, cosmo):
    ...
    a = np.exp(ln_a)
    alpha = 3.0 / (5.0 - cosmo.wQI / (1.0 - cosmo.wQI)) + 3.0 / 125.0 * (
        1.0 - cosmo.wQI
    ) * (1.0 - 3.0 * cosmo.wQI / 2.0) / (1.0 - 6.0 * cosmo.wQI / 5.0) ** 3 * (
        1.0 - cosmo.omegamI
    )
...
def omegamAA(a, cosmo):
    return cosmo.omegamI / (
        cosmo.omegamI
        + cosmo.omegakI * a
        + cosmo.omegavI * a**3
        + cosmo.omegaQI / a ** (3.0 * cosmo.wQI)
    )
...
def fgQQ(z, power):
    ...
    if type(z) is float:
        if z == 0:  # This gets called often so np.interp is too slow for this case.
            # Need to find out what is calling so often
            fgQQ_value = power.f_growth[0]
    else:
        fgQQ_value = np.interp(z, power.z_growth, power.f_growth)

    # for i in range(power.num_growth):
    #     if z >= power.z_growth[i] and z <= power.z_growth[i + 1]:
    #         if (power.z_growth[i + 1] - power.z_growth[i]) > zatol:
    #             wL = (z - power.z_growth[i]) / (
    #                 power.z_growth[i + 1] - power.z_growth[i]
    #             )
    #             wR = (power.z_growth[i + 1] - z) / (
    #                 power.z_growth[i + 1] - power.z_growth[i]
    #             )
    #         else:
    #             wL = 0.0
